...
export function NotificacionesMenu({ sucursalId }: { sucursalId: string }) {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      const { data: totalAlertas } = await supabase.rpc(
        "contar_alertas_stock",
      ...
      if (typeof totalAlertas === "number" && totalAlertas > 0) {
        notifs.push({
          id: "stock-bajo",
          tipo: "stock",
          titulo: `${totalAlertas} producto${totalAlertas !== 1 ? "s" : ""} con stock bajo`,
    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        {notificaciones.length > 0 && (
          <span
            className="absolute -top-1 -right-1 min-w-3.75 h-3.75 px-1
      ...
      {abierto && (
        <div
          className="absolute right-0 top-full mt-2 w-72 bg-surface border border-border
