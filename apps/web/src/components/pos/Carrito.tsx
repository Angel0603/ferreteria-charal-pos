...
import { Plus, Minus, Trash2, User, FileText, Receipt, Banknote, CreditCard, Landmark, Shuffle, HandCoins, Tag } from 'lucide-react'
import { formatCurrency } from '@/lib/utils'
import Image from 'next/image'
import { Package } from 'lucide-react'
import type { ItemCarrito, MetodoPago } from './useCarrito'
import { ModalDescuentoProducto } from './ModalDescuentoProducto'
...
type Props = {
  ...
  onDescuentoProducto: (productoId: string, descuento: number) => void
...
const METODOS: { value: MetodoPago; label: string; icon: React.ReactNode }[] = [
  ...
  { value: 'transferencia', label: 'Transferencia',  icon: <Landmark size={15} /> },
  { value: 'mixto',         label: 'Mixto',          icon: <Shuffle size={15} /> },
]

export function Carrito({
  items, subtotal, descuento, total, metodoPago, clienteId, clienteNombre,
  onCantidad, onEliminar, onDescuentoProducto, onMetodoPago, onCobrar, onCliente, onDescuento,
}: Props) {
  ...
  return (
    <div className="h-full flex flex-col bg-surface border-l border-border">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-dashed border-border-strong shrink-0">
        <div className="flex items-center justify-between mb-2.5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-medium text-text-primary">Venta actual</h2>
          </div>
        ...
        >
          ...
          <span className="text-xs font-mono bg-surface text-text-tertiary
                           px-1.5 py-0.5 rounded border border-border">F4</span>
        </button>
      </div>
          ...
          <div className="divide-y divide-border">
            {items.map(item => (
              <div key={item.producto.id} className="px-4 py-3 flex gap-3">
                <div className="w-10 h-10 rounded-lg bg-surface-2 border border-border
      ...
      {/* Footer */}
      <div className="border-t border-dashed border-border-strong p-4 space-y-3 shrink-0 bg-surface-2">
        <div className="space-y-1.5">
          <div className="flex justify-between text-sm">
            <span className="text-text-secondary">Subtotal</span>
            <span className="text-text-primary font-mono">{formatCurrency(subtotal)}</span>
          </div>
          <button
            onClick={onDescuento}
          ...
          >
            <span className="text-text-secondary flex items-center gap-1.5">
              Descuento global
              <span className="text-xs font-mono bg-surface text-text-tertiary
                               px-1.5 py-0.5 rounded border border-border">F8</span>
            </span>
            <span className={descuento > 0 ? 'text-success font-mono' : 'text-text-tertiary'}>
        ...
        <button
          onClick={onMetodoPago.bind(null, 'credito')}
          disabled={!clienteId}
          title={!clienteId ? 'Selecciona un cliente para vender a crédito' : undefined}
      ...
      {itemDescuento && (
        <ModalDescuentoProducto
          nombreProducto={itemDescuento.producto.nombre}
          precioUnitario={itemDescuento.precio_unitario}
          cantidad={itemDescuento.cantidad}
          descuentoActual={itemDescuento.descuento}
          onAplicar={(descuento) => {
            onDescuentoProducto(itemDescuento.producto.id, descuento)
            setItemDescuento(null)
          }}
          onClose={() => setItemDescuento(null)}
        />
      )}
    </div>
  )
