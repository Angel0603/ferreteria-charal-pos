...
export default function PerfilPage() {
  ...
  function nivelPassword(pwd: string) {
    ...
    if (pwd.length < 8)
      return { label: "Débil", color: "bg-danger", ancho: "w-1/3" };
    if (pwd.length < 12)
      return { label: "Aceptable", color: "bg-warning", ancho: "w-2/3" };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-surface rounded-xl border border-border p-6 flex items-center gap-5">
        <div
          className="w-16 h-16 rounded-full bg-accent-soft flex items-center
              ...
              {nivel && (
                <div className="flex items-center gap-1.5 mt-1.5">
                  <div className="flex-1 h-1 rounded-full bg-surface-2 overflow-hidden">
                    <div
                      className={`h-full transition-all ${nivel.color} ${nivel.ancho}`}
                    />
                  </div>
